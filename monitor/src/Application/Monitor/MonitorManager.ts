...
export default class MonitorManager {
  ...
  private readonly monitorsByConfiguration: Map<number, number[]> = new Map<number, number[]>()
  private monitors: Record<string, Monitor> = {}
  private checkers: Record<string, Checker> = {}
  ...
  public run (): void {
    ...
    Object.values(this.monitors).forEach((monitor: Monitor) => {
  ...
  public updateCheck (monitor: Monitor): void {
    this.checkers[monitor.id].updateMonitor(monitor)
  }
  ...
  public startCheck (monitor: Monitor): void {
    if (!(monitor.id in this.checkers)) {
  ...
  private async handleMonitorEvent (id: number, event: 'monitor_updated' | 'monitor_disabled' | 'monitor_enabled'): Promise<void> {
