...
import CheckSignMissCommand from "./CheckSignMissCommand";
...
export default class CheckSignMissCommandHandler implements CommandHandler {
    ...
    async handle(command: CheckSignMissCommand): Promise<CheckResult> {
        const currentMissCounter = await (await this.getClient(command.configuration)).fetchMissCounter(command.valoperAddress)

        if (command.lastState === undefined) {
            return new CheckResult(
                CheckStatus.OK,
                'No previous state found. Starting fresh.',
                CheckSignCommandState.fresh(currentMissCounter)
            )
        }
        ...
        if (currentMissCounter > command.lastState.lastNumberOfBlocksMissed) {
            ...
            if (missDifference >= command.missTolerance) {
                return new CheckResult(
                    CheckStatus.ERROR,
                    `Missed too many signing blocks. Miss counter: ${missDifference}. Miss tolerance: ${command.missTolerance}`,
