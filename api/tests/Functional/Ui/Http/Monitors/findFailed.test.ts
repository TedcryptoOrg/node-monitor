...
describe('Monitors find all failed controller', () => {
    ...
    it('use pagination', async () => {
        const result = await request(server)
            .get('/api/monitors/failed?offset=0&limit=1')
            .expect(200)
        ...
        const result2 = await request(server)
            .get('/api/monitors/failed?offset=1&limit=1')
            .expect(200)

        expect(result2.body.length).toBe(1)
        expect(result2.body[0].id).toBe(failed2.id)
        expect(result2.body[0].name).toBe(failed2.name)
    })
});
