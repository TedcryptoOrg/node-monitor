...
export class NodeMonitor extends AbstractMonitor {
  constructor (
    ...
    if (this.configuration.node_exporter !== undefined && this.configuration.node_exporter.enabled) {
      this.monitor_params.push(
        new UrlCheck(this.name, 'NodeExporter', this.configuration.node_exporter.address, this.alertChannels)
      )
      if (this.configuration.node_exporter.alerts?.diskSpace !== undefined && this.configuration.node_exporter.alerts.diskSpace.enabled) {
        this.monitor_params.push(
